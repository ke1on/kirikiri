    ...
    <div class="register bg-[var(--textColorWhite)] w-full" @submit="onsubmit">
        ...
        <div class="inputBox  ">
            <div class="item">
                ...
                <input type="text" @click="$emit('setClose',false)" v-model="state.useID" name="useID" id="useID" placeholder="请输入账号，英文数字组合" />
            </div>
            <div class="item" v-if="modle == 0">
                <label for="useName">用户名：</label>
                <input type="text" @click="$emit('setClose',false)" v-model="state.useName" name="useName" id="useName" placeholder="请输入用户名，昵称" />
            </div>
            <div class="item">
                <label for="password">密<span class="nbsp"></span>码：</label>
                <input type="password" @click="$emit('setClose',true)" v-model="state.password" name="password" id="password"
